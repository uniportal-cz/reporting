...
type Item = Section1Type['sample'][number]

interface Props { data: Section1Type; date: string }
...
export default function Section1({ data, date }: Props) {
  const { filtered, query, setQuery } = useTableFilter(data.sample, ['admin', 'nazev', 'skupina_nazev', 'typ'])
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-3">
        <input
          type="text"
          placeholder="Filtr podle osoby, názvu, skupiny, typu..."
          value={query}
        ...
        />
        {data.sample.length < data.total && (
          <span className="flex-shrink-0 rounded-full bg-amber-100 px-2.5 py-0.5 text-xs font-medium text-amber-700">
            Zobrazeno {data.sample.length} z {data.total}
          </span>
        )}
        <a href={`/api/reports/${date}/export?section=1`} className="flex-shrink-0 rounded bg-gray-100 px-3 py-1.5 text-sm font-medium text-gray-700 hover:bg-gray-200">
          Export CSV
        </a>
      </div>
        ...
        <table className="min-w-full text-sm">
          <thead>
            <tr className="bg-gray-50 text-xs font-semibold uppercase tracking-wider text-gray-500">
              ...
              <SortTh col="typ" label="Typ" sortKey={sortKey} sortDir={sortDir} onSort={toggleSort} />
