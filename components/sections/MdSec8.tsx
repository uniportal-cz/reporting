...
import { MdSec8 as MdSec8Type } from '@/types/report'
import StatBars from './StatBars'

interface Props { data: MdSec8Type; date: string }

export default function MdSec8({ data }: Props) {
  const sorted = [...data.items].sort((a, b) => b.pocet - a.pocet)

  return (
    <div className="space-y-4">
      {Object.keys(data.stats.bySablona).length > 0 && (
        <StatBars data={data.stats.bySablona} title="Dle šablony" colorClass="bg-orange-400" />
      )}
