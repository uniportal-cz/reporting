...
async function main() {
  ...
  console.log(`sec1  (Doprodej bez zásoby):      ${status(!!s.sec1, s.sec1?.items.length)}`)
  console.log(`sec2  (Saleable bez dodavatele):   ${status(!!s.sec2, s.sec2?.dodavatele.reduce((n, d) => n + d.produkty.length, 0))}`)
  console.log(`sec3  (WithVariant rozdílná cena): ${status(!!s.sec3, s.sec3?.items.length)}`)
